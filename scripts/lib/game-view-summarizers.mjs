export function summarizeButton(button) {
  return {
    id: button.id,
...
export function summarizeRelicLabel(relic) {
  return relic.count == null ? relic.label : `${relic.label} (${relic.count})`;
}

export function summarizeRelic(relic) {
  return {
    id: relic.id,
    label: relic.label,
...
export function summarizeMenuItem(item) {
  return {
    id: item.id,
    label: item.label,
...
export function summarizeCombatCard(card) {
  return {
    id: card.id,
    title: card.title,
...
export function summarizeCostChange(change) {
  return {
...
export function summarizePotion(potion) {
  return {
    id: potion.id,
...
export function summarizeCreature(creature) {
  return {
    id: creature.id,
    name: creature.name,
    side: creature.side,
...
export function summarizeCombatActionState(state) {
  if (!state) {
    return null;
  }
...
export function summarizeCardBrowse(cardBrowse) {
  return {
...
export function summarizeRunHistory(runHistory) {
  return {
